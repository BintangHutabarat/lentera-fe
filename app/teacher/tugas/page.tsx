import { PageTopbar } from "@/components/layout/PageTopbar";
import { ClipboardList } from "lucide-react";

export default function TeacherTugasPage() {
  return (
    <>
      <PageTopbar title="Tugas" subtitle="Kelola tugas siswa" />
      <div className="px-3.5 pt-3.5">
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <div className="w-16 h-16 rounded-full bg-teal-light flex items-center justify-center mb-4">
            <ClipboardList size={28} className="text-brand-teal" />
          </div>
          <h3 className="text-[15px] font-extrabold text-ink mb-2">Manajemen Tugas</h3>
          <p className="text-[12px] text-ink-muted max-w-[240px] leading-relaxed">
            Fitur buat dan kelola tugas sedang dalam pengembangan. Segera hadir!
          </p>
        </div>
      </div>
    </>
  );
}
